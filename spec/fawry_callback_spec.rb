...
RSpec.describe Fawry::FawryCallback do
  describe '#parse' do
    it 'wraps fawry server callback into FawryCallback instance' do
      fawry_callback = described_class.new(fawry_callback_params, 'fawry_secure_key', {}).parse
    ...
    context 'request signature' do
      it 'does not raise error when signature is valid' do
        expect do
          described_class.new(fawry_callback_params,
                              'fawry_secure_key', {}).parse
        end.to_not raise_error
      end

      it 'raises InvalidSignatureError if signature cannot be verified' do
        expect do
          described_class.new(fawry_callback_params.merge(messageSignature: 'abc'),
                              'fawry_secure_key', {}).parse
